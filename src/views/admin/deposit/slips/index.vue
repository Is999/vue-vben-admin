            ...
            :actions="[
              {
                ...
                disabled: !hasPermission(PermissionsEnum.DepositDetails, false),
              },
            ]"
          />
        </template>
...
<script setup lang="ts">
  ...
  import { getSlipsList } from '/@/api/admin/deposit';
  import { Tag, Tooltip } from 'ant-design-vue';
  import { usePermission } from '/@/hooks/web/usePermission';
  import { PermissionsEnum } from '/@/enums/roleEnum';
  import {
    getChannelsSelectList,
    getGatewaySelectList,
    getPlatformsSelectList,
  ...
  const columns: BasicColumn[] = [
    {
      ...
      fixed: 'left',
      customRender: ({ record }) => {
        if (!hasPermission(PermissionsEnum.DepositDetails, false)) {
          return record.no;
        }
    ...
    {
      title: 'BB Actual Amount',
      dataIndex: 'bb_actual_amount',
      width: 180,
      resizable: true,
      sorter: true,
    },
    ...
    },
    {
      ...
      resizable: true,
      customRender: ({ record }) => {
        return record.currency_type || 'rmb';
      },
    },
    {
      title: 'Rank',
  ...
  const [registerTable, { reload, setColumns, getColumns }] = useTable({
    ...
    actionColumn: {
      ...
      dataIndex: 'actionCol',
      fixed: 'right',
      ifShow: hasPermission(PermissionsEnum.DepositDetails, false),
    },
  });
